...
const gamesID = config.get('discord.gamesID');
...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
  ...
  function message(user, userID, channelID, message, pm) {
    if (!gamesID.indexOf(channelID)) { 
      ...
      } } else chat(channelID, '<@'+userID+'>: This command only works in <#'+gamesID+'>.');
}
  ...
  class Mini {
    ...
    newRound(id, c, prompt) {
      ...
      this.responseTimeTest = this.responseTime/60*100
      if (this.responseTimeTest = 60) { this.chat('**Round '+this.round+'** prompt ('+this.getAlivePlayers(true).length+' players):\n**'+prompt+'**\n'+
                'You have '+Math.round(this.responseTime/60*100)/100+' minute!\n'+
                'If you are a player, send me a private message: `!mini submit <response>`\n'+
                'e.g. `!mini submit Anything relating to the prompt in ten words or fewer!`'); }
      else if (this.responseTimeTest != 60) { this.chat('**Round '+this.round+'** prompt ('+this.getAlivePlayers(true).length+' players):\n**'+prompt+'**\n'+
    ...
    newVoting() {
      ...
      this.voteTimeTest = this.voteTime/60*100
      if (this.voteTimeTest = 60) { this.chat('**Round '+this.round+'** voting has begun!\n**Prompt: '+this.prompt+'**\n'+
      ...
      'There is '+Math.round(this.voteTime/60*100)/100+' minute to vote! Get going!\n'+
      'If you did NOT get the options, use `!mini options`. Sometimes options can take a little bit to get sent to you, though.'); }
      else if (this.voteTimeTest != 60) { this.chat('**Round '+this.round+'** voting has begun!\n**Prompt: '+this.prompt+'**\n'+
      (noResponse.length ? 'I did not receive a response from: '+noResponse.map(player => '<@'+player.id+'>').join(', ')+'\n' : '')+
      'Remember, you viewers can vote whether you\'re a contestant or not!\n'+
      'If you\'re not a player or spectator, to vote use `!mini spec`\n'+
